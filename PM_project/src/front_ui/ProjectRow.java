...
public class ProjectRow extends JPanel {
	...
	public void setTagChips(ArrayList<Tag> tagList) {
		panelTag.removeAll();
		List<TagChip> tagchipArray = new ArrayList<TagChip>();
		if (tagList == null)
			return;
		for (int i = 0; i < tagList.size(); i++) {
			tagchipArray.add(new TagChip(tagList.get(i).getName(), tagList.get(i).getColor(), Color.BLUE, Math.min(140, 52 + tagList.get(i).getName().length() * 10), TAG_H));
		}
		int leftW = panelTag.getWidth();
		final int CHIP_H = 28;
		int cx = PAD, cy = (TAG_H - CHIP_H) / 2, cg = 8;
		if (tagList != null) {
			//			w = Math.min(140, 52 + tag.getName().length() * 10)
			for (TagChip tagchip : tagchipArray) {
				//				tagchip.setSize(tagchip.getWidth(), CHIP_H);
				tagchip.setLocation(cx, cy);
				panelTag.add(tagchip);
				cx += tagchip.getWidth() + cg;
				if (cx + 64 > leftW - PAD)
					break;
			}
		}
