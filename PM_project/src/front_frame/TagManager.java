...
public class TagManager extends BasePage {
    ...
    public TagManager() {
        super(new TopBar.OnMenuClick() {
            @Override public void onClass()   { DefaultFrame.getInstance(new ClassManager()); }
            @Override public void onStudent() { DefaultFrame.getInstance(new StudentManager()); }
            @Override public void onTag()     { DefaultFrame.getInstance(new TagManager()); }
        });
        ...
        tabs = new TabsBar(
            new TabSpec[] {
                new TabSpec("학생",   Theme.TAB_OFF_BG),   // 산출물 탭 색 변경은 Theme class에서
    ...
    private void applyVisibility() {
        if (studentList == null || outputList == null) return; // 안전 가드
    ...
    private void addNewTagDialog() {
        ...
        switch (colorChoice) {
            ...
            case "회색": color = Color.DARK_GRAY; break;
            case "노랑": color = Color.ORANGE; break;
            case "직접 선택":
    ...
    private void editTagDialog(boolean toStudent, TagRow row) {
        ...
        if (newRemark.isEmpty()) newRemark = "-";

        // 선택한 색상 처리
        String colorChoice = (String) colorCombo.getSelectedItem();
        Color newColor;
        switch (colorChoice) {
            case "파랑": newColor = Color.BLUE; break;
            case "빨강": newColor = Color.RED; break;
            case "회색": newColor = Color.DARK_GRAY; break;
            case "노랑": newColor = Color.ORANGE; break;
            case "직접 선택":
